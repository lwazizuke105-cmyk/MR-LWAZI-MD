...
// ── New Feature Imports ──────────────────────────────────────────────────────
const { antistatusCommand, handleAntiStatus } = require('./commands/antistatus');
const { morabarabaCommand } = require('./commands/morabaraba');
const { appDownloadCommand } = require('./commands/appdownload');
const { manusDownloadCommand } = require('./commands/manusdownload');
const { groupTimeCommand } = require('./commands/grouptime');
const { getGroupTimeHeader } = require('./lib/grouptime');

// Global settings
global.packname = settings.packname;
global.author = settings.author;
global.channelLink = "https://whatsapp.com/channel/0029VbDK7drI1rcoEQNE1K3S";
...
async function handleMessages(sock, messageUpdate, printLog) {
    try {
        ...
        // 🕒 Auto-show group time at top of bot responses in groups
        // (Stored so we can prepend to responses - used by help/menu commands)
        if (isGroup) {
            try {
                const { getGroupTimezone } = require('./commands/grouptime');
                const tz = getGroupTimezone(chatId);
                global._groupTimeHeader = getGroupTimeHeader(tz);
            } catch (e) {
                global._groupTimeHeader = getGroupTimeHeader('Africa/Johannesburg');
            }
        }
        ...
        switch (true) {
            ...
            case userMessage.startsWith('.time'):
                {
                    const adminStatusTime = isGroup ? await isAdmin(sock, chatId, senderId) : { isSenderAdmin: false };
                    await groupTimeCommand(sock, chatId, message, senderId, adminStatusTime.isSenderAdmin);
                }
                commandExecuted = true;
                break;
            // 💀 Antistatus
            case userMessage.startsWith('.antistatus'):
                {
                    ...
                    const adminStatusAs = await isAdmin(sock, chatId, senderId);
                    await antistatusCommand(sock, chatId, message, senderId, adminStatusAs.isSenderAdmin);
                }
                commandExecuted = true;
                break;
            // 🎯 Morabaraba Game
            case userMessage.startsWith('.morabaraba') || userMessage.startsWith('.mora') || userMessage.startsWith('.umlabalaba'):
                await morabarabaCommand(sock, chatId, message, senderId);
                commandExecuted = true;
                break;
            // 📱 App Download
            case userMessage.startsWith('.apk') || userMessage.startsWith('.app ') || userMessage === '.app' || userMessage.startsWith('.appinfo') || userMessage.startsWith('.appsearch'):
                await appDownloadCommand(sock, chatId, message);
                commandExecuted = true;
                break;
            // 🤖 Manus AI Download Assistant
            case userMessage.startsWith('.manus') || userMessage.startsWith('.manusdownload') || userMessage.startsWith('.manushelp'):
                await manusDownloadCommand(sock, chatId, message);
                commandExecuted = true;
                break;
            default:
                if (isGroup) {
